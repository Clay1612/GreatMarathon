...
currentDate.textContent = _dateFns.format(new Date(), "do MMM");
for (let task of _viewJs.inputs.addNewTask)task.addEventListener('submit', function() {
    let currentTask = undefined;
    if (task.classList.contains('high-priority')) {
        currentTask = _viewJs.createNewTask(_viewJs.inputs.newTaskValue[0].value);
        task.after(currentTask);
        _viewJs.inputs.newTaskValue[0].value = '';
    } else if (task.classList.contains('low-priority')) {
        currentTask = _viewJs.createNewTask(_viewJs.inputs.newTaskValue[1].value);
        task.after(currentTask);
        _viewJs.inputs.newTaskValue[1].value = '';
    }
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["kQMTH","adjPd"], "adjPd", "parcelRequire23f1")
